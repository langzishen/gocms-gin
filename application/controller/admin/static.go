...
func (sc *StaticController) PostList(ctx *gin.Context) {
	conf := config.InitConfig()
	static_root := conf.Upload.UploadTypeConf.RootPath
	dir := ""
	search := ""
	keyPrefix := ctx.PostForm("keyPrefix")
	if keyPrefix != "" {
		dir = filepath.Join(static_root, keyPrefix)
	} else {
		dir = static_root
	}
	if ctx.PostForm("search") != "" {
	...
	ctx.JSON(http.StatusOK, map[string]interface{}{"maxKeys": 100, "keyPrefix": ctx.PostForm("keyPrefix"), "rows": list})
}

func (sc *StaticController) PostAdddir(ctx *gin.Context) {
	ctx.HTML(http.StatusOK, "admin/"+RequestController+"/adddir.html", ViewData)
}

//创建文件夹
func (sc *StaticController) PostDoadddir(ctx *gin.Context) {
	dir_name := ctx.PostForm("dir_name")
	keyPrefix := ctx.PostForm("keyPrefix")
...
文件上传
*/
func (sc *StaticController) PostMultiupload(ctx *gin.Context) {
	keyPrefix, _ := ctx.Cookie("keyPrefix")
	var err error
	if keyPrefix == "" {
...
删除文件或空文件夹
*/
func (sc *StaticController) PostDelfile(ctx *gin.Context) {
	uuid := ctx.PostForm("uuids")
...
下载
*/
func (sc *StaticController) GetDownload(ctx *gin.Context) {
	uuid := ctx.Query("uuid")
	if uuid == "" {
