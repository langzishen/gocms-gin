...
var MaxMultipartMemory *gin.Engine
...
*/
func Upload(ctx *gin.Context, post_file_name string, UploadPath ...string) (url_path string, Abs_path string, err error) {
	app_conf := config.InitConfig()
	if app_conf.Upload.UploadTypeConf.MaxSize != 0 {
		MaxMultipartMemory.MaxMultipartMemory = 0 << app_conf.Upload.UploadTypeConf.MaxSize //设置最大上传的文件的大小
	}
	file, err := ctx.FormFile(post_file_name)
	if err != nil {
		return "", "", err
	}
