...
*/
func (r *RBAC) SaveAccessList(authId uint) {
	r = initRBAC()
	if is_admin, _ := app_session.GetBoolean(r.AdminAuthKey); r.UserAuthType != 2 && !is_admin {
		accessMap := r.GetAccessMap(authId)
		if len(accessMap) == 0 {
			accessMap = make(map[string]map[string]map[string]string)
		}
		accessMapjSON, _ := json.Marshal(accessMap)
		app_session.Set("_ACCESS_LIST", string(accessMapjSON))
	}
}
...
// 权限认证的过滤器方法
func (r *RBAC) AccessDecision(request_app, request_controller, request_action string) bool {
	r = initRBAC()
	// 检查是否需要认证
	if r.CheckAccess(request_controller, request_action) {
		...
		if isok, _ := app_session.GetBoolean(r.AdminAuthKey); !isok {
			var accessMap map[string]map[string]map[string]string
			if r.UserAuthType == 1 { //登录验证
				accessMap = make(map[string]map[string]map[string]string)
				accessInterface := app_session.GetString("_ACCESS_LIST")
				//accessInterface_b, _ := json.Marshal(accessInterface)
				accessInterface_b := []byte(accessInterface)
				json.Unmarshal(accessInterface_b, &accessMap)
			} else {
				//r.UserAuthType == 2
				// 加强验证和即时验证模式 更加安全 后台权限修改可以即时生效
				// 通过数据库进行访问检查
				authId, _ := app_session.GetInt(r.UserAuthKey)
			...
			if accessMap[strings.ToUpper(request_app)][strings.ToUpper(request_controller)][strings.ToUpper(request_action)] != "" {
				app_session.Set(accessGuid, true)
			} else {
				app_session.Set(accessGuid, false)
				return false
			}
		} else { //超级管理员无需认证
			return true
